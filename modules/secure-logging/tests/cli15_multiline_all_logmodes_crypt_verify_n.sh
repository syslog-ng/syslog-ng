...
# File:   cli15_multiline_all_logmodes_crypt_verify_n.sh
# Date:   2026-05-29
#
...
#-----------------------------------------------------------------------
# -- helper function to check output file by tee of verification of normal
#    mode.
# Usage: check_verify_normal "path_slog" "path_slog_verified" "path_tee"

check_verify_normal() {
    echo " "
    echo "----------------------------------------"
    echo "--- check_verify_normal"
    echo "----------------------------------------"
    echo " "
    ...
    # e.g.: "${TEST}/${INPUT_FILE}.slog_verified.txt"
    local path_slog_verified="$2"

    # This full file name is the console log of slogverify
    # captured by tee
    # e.g.: "${TEST}/slogverify-normal-mode-result.log"
    local path_tee="$3"
    ...
    if grep -q "There is a problem with log verification. Please check log manually" "${path_tee}"; then
        cnt_error=$((cnt_error + 1))
        return 1
    fi
...
while [[ ${k} -lt ${TOTAL_LOG_MODES} ]]; do
    ...
    echo " "
    echo "----------------------------------------"
    echo "--- slogverify NORMAL MODE, logmode: ${current_logmode}"
    echo "--- Loop i: ${i}, j: ${j}"
    echo "----------------------------------------"
    echo " "
    ...
    if [[ ${IS_BIN_SUPPORT_PLAIN} == "true" ]]; then
        "${BIN}/slogverify" \
            ...
            --logmode "${current_logmode}" \
            "${TEST}/${INPUT_FILE}_${j}.slog" \
            "${TEST}/${INPUT_FILE}.slog_verified_${j}.txt" 2>&1 | tee "${TEST}/slogverify-normal-mode-result_${j}.log"
        ret=${PIPESTATUS[0]}
        if ((ret != 0)); then
            cnt_error=$((cnt_error + 1))
    ...
    check_verify_normal \
        "${TEST}/${INPUT_FILE}_${j}.slog" \
        "${TEST}/${INPUT_FILE}.slog_verified_${j}.txt" \
        "${TEST}/slogverify-normal-mode-result_${j}.log"

    cp "${TEST}/${INPUT_FILE}.slog_verified_${j}.txt" "${TEST}/${INPUT_FILE}.slog_verified_${loops}_${current_logmode}.txt"
    cp "${TEST}/slogverify-normal-mode-result_${j}.log" "${TEST}/slogverify-normal-mode-result_${loops}_${current_logmode}.log"
