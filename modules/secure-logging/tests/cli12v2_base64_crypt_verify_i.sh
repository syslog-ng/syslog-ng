...
# File:   cli12v2_base64_crypt_verify_i.sh
# Date:   2026-05-29
#
...
#-----------------------------------------------------------------------
# Helper function to get the string length of the first line of a text file
...
get_first_line_length() {
    ...
    # Use parameter expansion to get the length
    echo "${#line}"
}

#-----------------------------------------------------------------------
# Helper function to get random number in the given range min and max
# Example usage:
# result=$(get_random 10 50)
# echo "Your random number is: $result"
...
if [[ ${cnt_error} -eq 0 ]]; then
    ...
    while [[ ${i} -lt ${MAX_LOOP} ]]; do
        ...
        linelen=$(get_first_line_length "${TEST}/plainlog_err_${i}.slog")
        echo "linelen: ${linelen}"
        # do not modify counter and colon here 12 + 1 = 13 idx 0..12
        random_position=$(get_random 13 linelen)
        echo "random_position: ${random_position}"
        random_byte=$(get_random 0 127)
        echo "random_byte: ${random_byte}"
        ...
        BASE64_BYTEWURST="${FIRST_LINE:13}"
        echo "BASE64_BYTEWURST:   ${BASE64_BYTEWURST}"

        # Replace iv-tag-msg Base64 String by another Base64 Stirng
        TAMPERED_BYTEWURST=$(./tamper_b64.sh "${BASE64_BYTEWURST}")
        echo "TAMPERED_BYTEWURST: ${TAMPERED_BYTEWURST}"

        NEW_TAMPERED="${LINE_COL}${TAMPERED_BYTEWURST}"
