...
while [ "${i}" -lt "${MAX_LOOP}" ]; do
    ...
    LOG_MESSAGE="${LOG_MESSAGE} The price is > 10€, < 20£, or ~ \$15."
    ...
    # cp "${TEST}/host.key" "${TEST}/h${i}.key"
    # cp "${TEST}/mac.dat" "${TEST}/mac${i}.dat"
    cp "${TEST}/host.key" "${TEST}/h${i_offset}.key"
    cp "${TEST}/mac.dat" "${TEST}/mac${i_offset}.dat"
    ...
    if ! [ -f "${TEST}/mac0.dat" ]; then
        ...
        echo "!!!"
        cnt_error=$((cnt_error + 1))
        cp "${TEST}/mac${i_offset}.dat" "${TEST}/mac0.dat"
    fi
    ...
    # hexdump current
    echo " "
    echo "hexdump -C ${TEST}/h${i_offset}.key"
    hexdump -C "${TEST}/h${i_offset}.key"

    echo " "
    echo "hexdump -C ${TEST}/mac${i_offset}.dat"
    hexdump -C "${TEST}/mac${i_offset}.dat"

done # while

# Get each encryption output as a file for iterative verification later
echo " "
echo "Store messages.slog.<n>.."
filename="${TEST}/messages.slog"
line_number=0
while IFS= read -r line; do
    line_number=$((line_number + 1))
    printf "%5d: %s\n" "${line_number}" "${line}"
    if [ -f "${TEST}/messages.slog.${line_number}" ]; then
        echo "Info: Overwrite existing ${TEST}/messages.slog.${line_number}"
    fi
    echo "${line}" >"${TEST}/messages.slog.${line_number}"
    if ! [ -f "${TEST}/messages.slog.${line_number}" ]; then
        cnt_error=$((cnt_error + 1))
        echo "ERROR: Does not exist: ${TEST}/messages.slog.${line_number}"
    fi
done <"${filename}"
# check line_number. When messages.slog is not valid increment error conter!
# The latest i_offset must be equla to line_number
echo " "
#if ((i_offset == line_number)); then
if [ "${i_offset}" -eq "${line_number}" ]; then
    printf "INFO: Found the expected number %d of lines in %s/messages.slog\n" "${line_number}" "${TEST}"
else
    printf "ERROR: Count of lines %d in %s/messages.slog is not expected %d!\n" "${line_number}" "${TEST}" "${i_offset}" >&2
    cnt_error=$((cnt_error + 1))
fi
...
echo "#-6"
echo "----------------------------------------"
echo "--- slogverify ITERATIVE MODE"
echo "----------------------------------------"
echo " "
...
while [ "${i}" -lt "${MAX_LOOP}" ]; do
    ...
    j=$((i - 1))
    i_offset=$((i + counter_value))
    j_offset=$((j + counter_value))
    echo " "
    echo " "
    echo "-- Loop i: ${i}, j: ${j} ---"
    echo "i: ${i}; counter_value: ${counter_value}; i_offset: ${i_offset}"

    echo " "
    echo "${BIN}/slogverify --iterative" \
        "--mac-file ${TEST}/mac${i_offset}.dat" \
        "-p ${TEST}/h${j_offset}.key" \
        "-r ${TEST}/mac${j_offset}.dat" \
        "${TEST}/messages.slog.${i_offset}" \
        "${TEST}/messages_check_${i_offset}.txt"

    echo " "
    "${BIN}/slogverify" --iterative \
        --mac-file "${TEST}/mac${i_offset}.dat" \
        -p "${TEST}/h${j_offset}.key" \
        -r "${TEST}/mac${j_offset}.dat" \
        "${TEST}/messages.slog.${i_offset}" \
        "${TEST}/messages_check_${i_offset}.txt" 2>&1 | tee "${TEST}/slogverify-iterative-mode-result${i_offset}.log"

    # -- check if test was successful -----
    echo " "
    # check if output files do exist
    if ! [ -f "${TEST}/messages.slog.${i_offset}" ]; then
        echo "ERROR: ${TEST}/messages.slog.${i_offset} does not exist"
        cnt_error=$((cnt_error + 1))
    else
        echo "${TEST}/messages.slog.${i_offset}"
        cat "${TEST}/messages.slog.${i_offset}"
    fi
    echo " "
    if ! [ -f "${TEST}/messages_check_${i_offset}.txt" ]; then
        echo "ERROR: ${TEST}/messages_check_${i_offset}.txt does not exist"
        cnt_error=$((cnt_error + 1))
    else
        echo "${TEST}/messages_check_${i_offset}.txt"
        cat "${TEST}/messages_check_${i_offset}.txt"
    fi
    echo " "
    ...
    if ! [ -f "${TEST}/slogverify-iterative-mode-result${i_offset}.log" ]; then
        ...
        if grep -q "Unable to recover" "${TEST}/slogverify-iterative-mode-result${i_offset}.log"; then
            cnt_error=$((cnt_error + 1))
        fi
...
while [ "${i}" -lt "${ALL}" ]; do
    i_offset=$((i + counter_value))
    echo " "
    echo "${BIN}/slogkey --counter ${TEST}/h${i_offset}.key"
    "${BIN}"/slogkey --counter "${TEST}/h${i_offset}.key"
    i=$((i + 1))
done
echo " "
echo "${BIN}/slogkey --counter ${TEST}/host.key"
...
while [ "${i}" -lt "${ALL}" ]; do
    i_offset=$((i + counter_value))
    sha256sum "${TEST}/mac${i_offset}.dat"
    i=$((i + 1))
done
...
while [ "${i}" -lt "${ALL}" ]; do
    i_offset=$((i + counter_value))
    # offset in case re-used host.key
    echo "Check if files mac${i_offset}.dat and h${i_offset}.key do exist .."
    if ! [ -f "${TEST}/mac${i_offset}.dat" ]; then
        echo "ERROR: ${TEST}/mac${i_offset}.dat does not exist"
        cnt_error=$((cnt_error + 1))
    fi
    if ! [ -f "${TEST}/h${i_offset}.key" ]; then
        echo "ERROR: ${TEST}/h${i_offset}.key does not exist"
        cnt_error=$((cnt_error + 1))
    fi
    i=$((i + 1))
done
