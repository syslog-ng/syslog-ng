...
if [ ${DO_USE_HARDCODED_PATH} = "true" ]; then
    # Normally the PREFIX is searched inside syslog-ng build artifacts.
    # These are not available when prebuild binaries are used and
    # copied to another system.
    # This script can provide therefore a hard coded path instead.
    #
    ...
    if [ -n "${CONFIG_FILE}" ]; then
        ...
        if [ -n "${CMAKE_INSTALL_FILE}" ]; then
            INSTALL_PATH=$(grep 'set(CMAKE_INSTALL_PREFIX' "${CMAKE_INSTALL_FILE}" | awk -F'"' '{print $2}')
