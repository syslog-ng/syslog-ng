...
# File:   cli11_enc_syslog_verify_n.sh
# Date:   2026-06-22
#
...
while [[ ${i} -lt ${MAX_LOOP} ]]; do
    ...
    echo " --- LOOP ${i} ------------"
    echo " "
    LOG_MESSAGE="abcde 12345 [INFO] This is log number ${i}: "
    LOG_MESSAGE="${LOG_MESSAGE} „Je größer der Dachschaden, desto schöner der Ausblick zum Himmel.“ Karlheinz Deschner."
    LOG_MESSAGE="${LOG_MESSAGE} \"Das fortwährende Dasein des Menschengeschlechts ist bloß ein Beweis der Geilheit desselben.\" Arthur Schoppenhauer."
    LOG_MESSAGE="${LOG_MESSAGE} 🤪 \"Wir haben dann einen Unterschied zwischen dem normal sich verhaltenden Bär,"
    LOG_MESSAGE="${LOG_MESSAGE} dem Schadbär und dem Problembär. Und, äh, es ist ganz klar, dass, äh, dieser Bär, äh, ein Problembär ist... ."
    LOG_MESSAGE="${LOG_MESSAGE} 🤪 ¿Qué pasa, señor? he price is > 100€, < 200£, or ~ \$150."
    LOG_MESSAGE="${LOG_MESSAGE} 🤪 Nachts ist es kälter aus draußen."
    LOG_MESSAGE="${LOG_MESSAGE} 🤪 Cola schmeckt besser als aus einem Glas."
    LOG_MESSAGE="${LOG_MESSAGE} 🤪 Zu Fuß ist es kürzer als über den Berg. "
    LOG_MESSAGE="${LOG_MESSAGE} 🤪 Kalte Brötchen schmecken warm besser als lecker."
    LOG_MESSAGE="${LOG_MESSAGE} 🤪 Drinnen ist es dunkler als die Straße hinunter."
...
if [[ ${IS_BIN_SUPPORT_PLAIN} == "true" ]]; then
    "${BIN}/slogverify" \
        ...
        1987 2>&1 | tee "${TEST}/slogverify-normal-mode-result.log"
