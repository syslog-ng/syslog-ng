...
Shell scripts are provided therefore and they can be called from inside unit tests.

Binaries will be updated from source by **make install**

Each test script uses temporarily the folder /tmp/test_slog/data_xxx/ and
...
Criterion unit test that also can be configured to call smoke tests by shell scripts.

### get_prefix.sh
Get the syslog-ng install directory which should be available when autogen.sh or cmake
build has successfully finished.
