...
# use new SW that supports logmode but use old config to check whether fall back --logmode enc is
# used instead
...
# BIN supports logmode but old variant is triggered. Check if fall back to --logmode enc is working.
