...
# File:   cli42_PROBLEM_LOOP_KEY_WHEN_MULITILINE.sh
# Date:   2026-03-26
#
...
if [[ ! -e ${PREFIX} ]]; then
    ...
    echo "Maybe the project has not yet been built or the build directory was deleted."
    echo "FAIL"
    exit 1
fi
...
i=0
while [[ ${i} -lt ${TOTAL_LOG_MODES} ]]; do
    current_logmode="${LOGMODES[${i}]}"
    # i: 1..3
    # j: 0..2
    i=$((i + 1))
    j=$((i - 1))
