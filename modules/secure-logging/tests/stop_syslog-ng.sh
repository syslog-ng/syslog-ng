...
# TODO clarify name
SYSLOGNGNAME="syslog-ng-main"
# SYSLOGNGNAME="syslog-ng"
...
stop_syslog() {
    ...
    if pgrep -x "${SYSLOGNGNAME}" >/dev/null; then
        echo "    syslog-ng is running and will be stopped now"

        # Send SIGTERM (standard stop signal)
        pkill -x "${SYSLOGNGNAME}"

        # Wait for the process to exit
        sleep "${STOP_WAIT_TIME}"

        # Double-check: If it's still there, force it
        if pgrep -x "${SYSLOGNGNAME}" >/dev/null; then
            echo "    Process did not stop; sending SIGKILL"
            pkill -9 -x "${SYSLOGNGNAME}"
        fi
    fi
