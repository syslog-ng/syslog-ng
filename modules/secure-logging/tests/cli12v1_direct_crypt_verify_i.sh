...
# File:   cli12v1_direct_crypt_verify_i.sh
# Date:   2026-05-29
#
...
if [[ ${cnt_error} -eq 0 ]]; then
    ...
    while [[ ${i} -lt ${MAX_LOOP} ]]; do
        ...
        random_position=$(get_random 54 linelen)
        echo "random_position: ${random_position}"
        random_byte=$(get_random 0 127)
        echo "random_byte: ${random_byte}"
        echo "modify_byte_v2 ${TEST}/plainlog_err_${i}.slog ${random_position} ${random_byte}"
        modify_byte_v2 "${TEST}/plainlog_err_${i}.slog" "${random_position}" "${random_byte}"
        ret=${PIPESTATUS[0]}
        if ((ret != 0)); then
            cnt_error=$((cnt_error + 1))
            echo "Failed to manipulate slog file, so the verification with a manipulated slog file is not possible! (rc=${ret})" >&2
        fi
