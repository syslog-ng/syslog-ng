...
get_decompress_command(GString *command, GString *compression_result, enum CurlCompressionTypes compressionType)
{
  ...
  switch (compressionType)
    ...
    {
      // NOTE: this requires the `gzip` package to be installed
      gchar* encoded = g_base64_encode((guchar *)compression_result->str, compression_result->len);
    ...
    {
      // NOTE: this requires the `qpdf` package to be installed
      gchar* encoded = g_base64_encode((guchar *)compression_result->str, compression_result->len);
