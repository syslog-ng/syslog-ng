...
get_decompress_command(GString *command, GString *compression_result, enum CurlCompressionTypes compressionType)
{
  ...
  switch(compressionType)
    ...
    case CURL_COMPRESSION_GZIP:
      g_string_printf(command, "echo %s | base64 -d | gunzip", g_base64_encode(compression_result->str, compression_result->len));
      break;
    case CURL_COMPRESSION_DEFLATE:
      g_string_printf(command, "echo %s | base64 -d | zlib-flate -uncompress", g_base64_encode(compression_result->str, compression_result->len));
      break;
    default:
      g_assert_not_reached();
    }
