...
GString** verifyMaliciousMessages(GString **templateOutput, size_t totalNumberOfMessages, int* brokenEntries) {
  ...
  memcpy(keyZero, hostkey, KEY_LENGTH);
    ...
    for (int i = 0; i<totalNumberOfMessages; i++) {
      ret = iterateBuffer(1, &templateOutput[i], &next, hostkey, keyZero, 0, &outputBuffer[i], &numberOfLogEntries, cmac_tag, &tab);
        if (ret == 0) {
...
void verifyMessages(GString **templateOutput, LogMessage **original, size_t totalNumberOfMessages) {
  ...
  struct hsearch_data tab;
    ...
    ret = iterateBuffer(totalNumberOfMessages, templateOutput, &next, hostkey, keyZero, 0, outputBuffer, &numberOfLogEntries, cmac_tag, &tab);
...
void test_slog_malicious_modifications() {
    ...
    for (int i = 0; i < mods; i++) {
        ...
        g_string_overwrite(output[entriesToModify[i]], randomNumber(COUNTER_LENGTH + COLON, (output[entriesToModify[i]]->len)-1), "999999999999999999999999999999999999999999999999999999999999999");
    }
