...
public class ElasticSearchOptions {
    ...
    public static HashSet<String> CLIENT_MODES = new HashSet<String>(Arrays.asList(CLIENT_MODE_TRANSPORT,
            CLIENT_MODE_NODE, CLIENT_MODE_SHIELD, CLIENT_MODE_HTTP, CLIENT_MODE_HTTPS, CLIENT_MODE_SEARCHGUARD));
    public static HashSet<String> HTTP_AUTH_TYPES = new HashSet<String>(
            Arrays.asList(HTTP_AUTH_TYPE_NONE, HTTP_AUTH_TYPE_BASIC, HTTP_AUTH_TYPE_CLIENTCERT));
    ...
    private void fillTemplateOptions() {
        options.put(new TemplateOption(owner.getConfigHandle(),
                new RequiredOptionDecorator(new StringOption(owner, INDEX))));
        options.put(new TemplateOption(owner.getConfigHandle(),
                new RequiredOptionDecorator(new StringOption(owner, TYPE))));
        options.put(new TemplateOption(owner.getConfigHandle(),
                new StringOption(owner, MESSAGE_TEMPLATE, MESSAGE_TEMPLATE_DEFAULT)));
    ...
    private void fillStringOptions() {
        ...
        options.put(new IntegerRangeCheckOptionDecorator(new StringOption(owner, FLUSH_LIMIT, FLUSH_LIMIT_DEFAULT), -1,
                Integer.MAX_VALUE));
        options.put(new EnumOptionDecorator(new StringOption(owner, CLIENT_MODE, CLIENT_MODE_DEFAULT), CLIENT_MODES));
        options.put(new EnumOptionDecorator(new StringOption(owner, HTTP_AUTH_TYPE, HTTP_AUTH_TYPE_DEFAULT),
                HTTP_AUTH_TYPES));
        options.put(new StringOption(owner, CONFIG_FILE));
        options.put(new IntegerRangeCheckOptionDecorator(
                new StringOption(owner, CONCURRENT_REQUESTS, CONCURRENT_REQUESTS_DEFAULT), 0, Integer.MAX_VALUE));
        ...
        options.put(new IntegerRangeCheckOptionDecorator(new StringOption(owner, FLUSH_TIMEOUT, FLUSH_TIMEOUT_DEFAULT),
                0, Integer.MAX_VALUE));
    }

    private void fillBooleanOptions() {
        options.put(new BooleanOptionDecorator(
                new StringOption(owner, SKIP_CLUSTER_HEALTH_CHECK, SKIP_CLUSTER_HEALTH_CHECK_DEFAULT)));
    }
}
