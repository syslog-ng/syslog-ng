...
static gboolean
systemd_sd_acquire_socket(AFSocketSourceDriver *s,
                          gint *acquired_fd)
{
  ...
  *acquired_fd = -1;
  ...
  if (number_of_fds > 1)
    ...
    {
      msg_error("Failed to acquire systemd sockets, disabling systemd source",
                NULL);
      return TRUE;
    }
  else
    {
      fd = systemd_sd_get_fd();
  ...
  if (*acquired_fd != -1)
    {
      ...
      msg_verbose("Acquired systemd socket",
                  evt_tag_int("systemd-sock-fd", *acquired_fd),
                  NULL);
      return TRUE;
    }
...
static gboolean
systemd_sd_acquire_socket(AFSocketSourceDriver *s, gint *acquired_fd)
{
  return TRUE;
}
#endif
...
SystemDSourceDriver*
systemd_sd_new(GlobalConfig *cfg)
{
  SystemDSourceDriver *self = g_new0(SystemDSourceDriver, 1);
  ...
  self->super.acquire_socket = systemd_sd_acquire_socket;
