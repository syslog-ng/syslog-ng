  ...
  if (fd != -1)
    {
      ...
      self->reader = (LogReader *)log_reader_new(log_proto_dgram_server_new(log_transport_streams_new(fd), &self->reader_options.proto_options.super));
      log_reader_set_options((LogPipe *)self->reader, s, &self->reader_options, 1, SCS_SUN_STREAMS, self->super.super.id, self->dev_filename->str);
