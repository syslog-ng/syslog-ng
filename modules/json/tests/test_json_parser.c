...
Test(json_parser, test_json_parser_different_type_arrays)
{
  ...
  assert_log_message_value_and_type_by_name(msg, ".prefix.dblarray", "1.234,1e+06,5.6789", LM_VT_LIST);
...
Test(json_parser, test_json_parser_compound_types_in_arrays_are_represented_as_json_encoded_strings)
{
  ...
  msg = parse_json_into_log_message("{'arrayofarrays': [[1,2],[3,4],[5,6]],"
                                    ...
                                    json_parser);
  assert_log_message_value_and_type_by_name(msg, ".prefix.arrayofarrays", "[[1,2],[3,4],[5,6]]", LM_VT_LITERAL);
  assert_log_message_value_and_type_by_name(msg, ".prefix.arrayofmixedtypes", "[\"str\",42,{},null]", LM_VT_LITERAL);
  assert_log_message_value_and_type_by_name(msg, ".prefix.arrayofobjects",
                                            "[{\"foo\":\"bar\",\"bar\":\"foo\"},{\"foo\":\"bar\",\"bar\":\"foo\"}]", LM_VT_LITERAL);
  log_msg_unref(msg);
  log_pipe_unref(&json_parser->super);
}
...
Test(json_parser, test_json_parser_extracts_array_elements_into_matches)
{
  ...
  assert_log_message_value_and_type_by_name(msg, "4", "{\"foo\":\"bar\"}", LM_VT_LITERAL);
  assert_log_message_value_and_type_by_name(msg, "5", "{\"bar\":\"foo\"}", LM_VT_LITERAL);
  log_msg_unref(msg);
  log_pipe_unref(&json_parser->super);
}
