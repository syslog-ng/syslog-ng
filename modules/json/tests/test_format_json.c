...
Test(format_json, test_format_json_order_duplicate_keys)
{
  cfg_set_version_without_validation(configuration, VERSION_VALUE_4_0);

  /* every order emits the last value of a duplicate name */
  assert_template_format("$(format-json --order=descending a=1 b=2 a=3)",
                         "{\"b\":\"2\",\"a\":\"3\"}");
  assert_template_format("$(format-json --order=ascending a=1 b=2 a=3)",
                         "{\"a\":\"3\",\"b\":\"2\"}");
  assert_template_format("$(format-flat-json --order=descending a=1 b=2 a=3)",
                         "{\"b\":\"2\",\"a\":\"3\"}");

  /* as-written keeps the first position of a duplicate name */
  assert_template_format("$(format-json --order=as-written a=1 b=2 a=3)",
                         "{\"a\":\"3\",\"b\":\"2\"}");
  assert_template_format("$(format-flat-json --order=as-written a=1 b=2 a=3)",
                         "{\"a\":\"3\",\"b\":\"2\"}");

  /* a duplicate leaf must not move its container */
  assert_template_format("$(format-json --order=as-written x.a=1 y=2 x.a=3)",
                         "{\"x\":{\"a\":\"3\"},\"y\":\"2\"}");
  assert_template_format("$(format-json --order=as-written x.a=1 y=2 x.b=3)",
                         "{\"x\":{\"a\":\"1\",\"b\":\"3\"},\"y\":\"2\"}");
}
