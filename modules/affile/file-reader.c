...
/* NOTE: Currently on Linux this will cause a huge number of logs if the ivykis poll methods are used - `follow-freq(0)`
         the first available regular file polling method is `ppol` (you have to exclude the not working epoll
         variants first to test this, IV_EXCLUDE_POLL_METHOD="epoll-timerfd epoll")
         ppoll/poll/etc. will always signal readability on regular file handles, so this check is called
         continuosly, and highly frequently.
         This is not true e.g. on macOS or any BSD variants, as there kqueue is available and will be the first
         selected ivikys poll method.
  TODO: We have to add a working kqueue alternative to the ivykis poll methods, e.g. using inotify
 */
static inline gboolean
