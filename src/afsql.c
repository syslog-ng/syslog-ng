...
static GList *
afsql_string_array_to_list(const gchar *strlist[])
{
  gint i;
  GList *l = NULL;
...
static void
afsql_string_list_free(GList *l)
{
  while (l)
    {
      g_free(l->data);
...
afsql_dd_set_columns(LogDriver *s, GList *columns)
{
  ...
  afsql_string_list_free(self->columns);
...
afsql_dd_set_indexes(LogDriver *s, GList *indexes)
{
  ...
  afsql_string_list_free(self->indexes);
...
afsql_dd_set_values(LogDriver *s, GList *values)
{
  ...
  afsql_string_list_free(self->values);
  self->values = values;
}
          ...
          if (dbi_result_get_field_idx(db_res, self->fields[i].name) == 0)
            {
              ...
              g_string_printf(query_string, "ALTER TABLE %s ADD COLUMN %s %s", table, self->fields[i].name, self->fields[i].type);
              if (!afsql_dd_run_query(self, query_string->str, FALSE, NULL))
                {
      ...
      if (self->pending_msg)
        {
          ...
          path_flags = self->pending_path_flags;
          self->pending_msg = NULL;
        }
      else
        {
          g_static_mutex_lock(&self->queue_lock);
          success = log_queue_pop_head(self->queue, &msg, &path_flags);
    ...
    error:
      ...
      if (success)
      ...
      else
        {
          ...
          self->pending_path_flags = path_flags;
          return FALSE;
        }
      count++;
    }
  ...
  while (!db_thread_terminate)
    {
      ...
      drivers = g_list_copy(sql_drivers);
...
{
  g_mutex_lock(sql_drivers_lock);
  sql_drivers = g_list_prepend(sql_drivers, dd);
  g_mutex_unlock(sql_drivers_lock);
}
...
{
  g_mutex_lock(sql_drivers_lock);
  sql_drivers = g_list_remove(sql_drivers, dd);
  g_mutex_unlock(sql_drivers_lock);
  afsql_thread_sync();
}
...
static gchar *
afsql_dd_format_stats_name(AFSqlDestDriver *self)
{
...
static gboolean
afsql_dd_init(LogPipe *s, GlobalConfig *cfg, PersistentConfig *persist)
{
  AFSqlDestDriver *self = (AFSqlDestDriver *) s;
  ...
  stats_register_counter(SC_TYPE_DROPPED, afsql_dd_format_stats_name(self), &self->dropped_messages, TRUE);
...
static gboolean
afsql_dd_deinit(LogPipe *s, GlobalConfig *cfg, PersistentConfig *persist)
{
  AFSqlDestDriver *self = (AFSqlDestDriver *) s;
  ...
  if (self->dropped_messages)
    stats_orphan_counter(SC_TYPE_DROPPED, afsql_dd_format_stats_name(self), &self->dropped_messages);
  
  return TRUE;
}

static void
afsql_dd_queue(LogPipe *s, LogMessage *msg, gint path_flags)
{
  AFSqlDestDriver *self = (AFSqlDestDriver *) s;
  gboolean consumed;
  
  g_static_mutex_lock(&self->queue_lock);
  consumed = log_queue_push_tail(self->queue, msg, path_flags);
  g_static_mutex_unlock(&self->queue_lock);
  g_cond_signal(db_thread_wakeup_cond);
  if (!consumed)
    {
...
afsql_dd_new()
{
  ...
  self->columns = afsql_string_array_to_list(default_columns);
  self->values = afsql_string_array_to_list(default_values);
  self->indexes = afsql_string_array_to_list(default_indexes);
