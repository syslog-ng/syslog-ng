...
afinter_source_new(AFInterSourceDriver *owner, LogSourceOptions *options)
{
  ...
  log_source_set_options(&self->super, options, 0, stats_components_get_component_index("internal"), owner->super.super.id, NULL, FALSE, FALSE,
  ...
  if (!internal_msg_queue)
    {
      ...
      stats_lock();
      stats_register_counter(0, stats_components_get_component_index("global"), "internal_queue_length", NULL, SC_TYPE_PROCESSED, &internal_queue_length);
      stats_unlock();
    }
