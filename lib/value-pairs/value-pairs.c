...
/* result_tree maps a name to its last index, so a repeated name keeps its first position and its last value */
static void
vp_foreach_in_insertion_order(VPResults *results, gpointer helper_args)
{
  GHashTable *emitted = g_hash_table_new(g_str_hash, g_str_equal);

  for (guint i = 0; i < results->values->len; i++)
    {
      VPResultValue *rv = &g_array_index(results->values, VPResultValue, i);

      if (!g_hash_table_add(emitted, rv->name->str))
        continue;

      gpointer last_ndx = g_tree_lookup(results->result_tree, rv->name->str);

      if (vp_foreach_helper(rv->name->str, last_ndx, helper_args))
        break;
    }

  g_hash_table_destroy(emitted);
}
 ...
 * vp_order_tree
 *
 * Used by the as-written walk. Name-value pairs are inserted in first-seen
 * order and replayed depth-first, so keys sharing a prefix stay contiguous as
 * the walker requires. A repeated name keeps its first position and takes its
 * last value.
 *******************************************************************************/
