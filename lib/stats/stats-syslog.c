  ...
  if (stats_check_level(3))
      ...
      for (i = 0; i < SEVERITY_MAX; i++)
        {
          ...
          stats_register_counter(3, stats_components_get_component_index("severity") | SCS_SOURCE, NULL, name, SC_TYPE_PROCESSED, &severity_counters[i]);
        }

      for (i = 0; i < FACILITY_MAX - 1; i++)
        {
          g_snprintf(name, sizeof(name), "%d", i);
          stats_register_counter(3, stats_components_get_component_index("facility") | SCS_SOURCE, NULL, name, SC_TYPE_PROCESSED, &facility_counters[i]);
        }
      stats_register_counter(3, stats_components_get_component_index("facility") | SCS_SOURCE, NULL, "other", SC_TYPE_PROCESSED,
  ...
  else
    {
      ...
      for (i = 0; i < SEVERITY_MAX; i++)
        {
          g_snprintf(name, sizeof(name), "%d", i);
          stats_unregister_counter(stats_components_get_component_index("severity") | SCS_SOURCE, NULL, name, SC_TYPE_PROCESSED, &severity_counters[i]);
        }

      for (i = 0; i < FACILITY_MAX - 1; i++)
        {
          g_snprintf(name, sizeof(name), "%d", i);
          stats_unregister_counter(stats_components_get_component_index("facility") | SCS_SOURCE, NULL, name, SC_TYPE_PROCESSED, &facility_counters[i]);
        }
      stats_unregister_counter(stats_components_get_component_index("facility") | SCS_SOURCE, NULL, "other", SC_TYPE_PROCESSED,
                               &facility_counters[FACILITY_MAX - 1]);
    }
  stats_unlock();
}
