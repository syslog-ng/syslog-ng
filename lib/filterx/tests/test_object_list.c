...
Test(filterx_list, test_filterx_object_list_marshals_to_the_stored_values)
{
  FilterXObject *fobj = filterx_list_new("foo,bar,baz", -1);
  assert_marshaled_object(fobj, "foo,bar,baz", LM_VT_LIST);
  filterx_object_unref(fobj);
}

Test(filterx_list, test_filterx_object_list_maps_to_the_right_list_value)
{
  FilterXObject *fobj = filterx_list_new("foo,bar,baz", -1);
  assert_object_json_equals(fobj, "[\"foo\",\"bar\",\"baz\"]");
  filterx_object_unref(fobj);
}
...
TestSuite(filterx_list, .init = setup, .fini = teardown);
