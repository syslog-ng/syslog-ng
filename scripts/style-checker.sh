...
{
    astyle --options="$root_dir/.astylerc" --dry-run "$root_dir/*.c" "$root_dir/*.h" \
    | grep "Formatted" | tee badly-formatted-files.list | wc -l \
    | while read badly_formatted_c_files
    do
...
{
    astyle --options="$root_dir/.astylerc" "$root_dir/*.c" "$root_dir/*.h" | grep "Formatted"
    exit 0
}
