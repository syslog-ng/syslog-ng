...
if [ $# -eq 1 ]; then
	if [ "$1" = "--update" ] ; then
		...
		rules=`ausearch --start $time -m avc --raw -se syslogd`
		if [ x"$rules" != "x" ] ; then
...
/sbin/restorecon -F -R -v /opt/syslog-ng/var/syslog-ng.persist
/sbin/restorecon -F -R -v /opt/syslog-ng/var/syslog-ng.persist-
...
echo "Please restart syslog-ng (as root) by the following command:"
echo "systemctl restart syslog-ng"
