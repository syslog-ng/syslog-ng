...
def test_manipulating_config_between_reload(config, syslog_ng):
    file_source, _ = config.create_file_source_and_writer(file_name="input.log")
    file_destination, _ = config.create_file_destination_and_reader(file_name="output.log")
    ...
    file_source2, _ = config.create_file_source_and_writer(file_name="input2.log")
    source_group2 = config.create_statement_group(file_source2)

    # create new file destination and update first destination group
    file_destination2, _ = config.create_file_destination_and_reader(file_name="output2.log")
