...
def test_set_tag(config, syslog_ng):
    ...
    file_destination, file_reader = config.create_file_destination_and_reader(file_name="output.log", template=config.stringify('${TAGS}\n'))
    config.create_logpath(statements=[generator_source, set_tag_with_matching, set_tag_without_matching, file_destination])

    syslog_ng.start(config)
    log_line = file_reader.read_log().strip()
