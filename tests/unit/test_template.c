...
test_compat(void)
{
  ...
  assert_template_format("$MSGHDR", "syslog-ng[23323]:");
  assert_grabbed_messages_contain("the default value for template-escape has changed to 'no' from syslog-ng PE 3.0", NULL);
  reset_grabbed_messages();
  assert_template_format("$MSG", "syslog-ng[23323]:árvíztűrőtükörfúrógép");
  assert_grabbed_messages_contain("the meaning of the $MSG/$MESSAGE macros has changed from syslog-ng PE 3.0", NULL);
  ...
  configuration->version = old_version;
}
